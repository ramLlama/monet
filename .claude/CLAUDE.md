...
8. **Lockfile protocol**: Lockfiles at `~/.claude/ide/<port>.lock` contain JSON with `pid`, `workspaceFolders`, `ideName`, `transport`, and `authToken`. Claude Code discovers these to connect. Windows uses `USERPROFILE` instead of `~`.
    ...
    - **`pid` defaults to `(emacs-pid)`, with an optional per-session LOCKFILE-PID override** (threaded through `monet-start-server-function` / `monet-start-server-in-directory` / `monet--create-lockfile`, parallel to PATH-MAPPINGS): Claude reaps lockfiles whose pid it cannot *signal*, and the two sides of a sandbox boundary disagree -- the Emacs pid does not exist in a guest's pid namespace (ESRCH), while pid 1 on a macOS host is unsignalable from userspace (`kill(1, 0)` = EPERM, which Claude treats as dead; verified 2026-06-12, **no pid validates in both namespaces**). So host sessions advertise `emacs-pid` (the default) and sandboxed callers pass 1 (alive in the guest, where the check actually runs). Reap triggers: every interactive host claude startup and `/ide` (directory-wide); headless `-p` never reaps.
